...
pub async fn log_service(
    ...
    if include_input {
        if let Some(validated_input) = log_data.get("validated_input") {
            ...
            input_table.set_header(vec![
                Cell::new("Input").add_attribute(comfy_table::Attribute::Bold)
            ]);
    ...
    if include_timer {
        ...
        if let Some(elapsed_time) = log_data.get("elapsed_time") {
            timer_table.add_row(vec![
                Cell::new("Elapsed Time"),
                Cell::new(elapsed_time.to_string()).set_alignment(CellAlignment::Center),
            ]);
        }
